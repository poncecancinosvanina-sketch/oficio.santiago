...
export default function GoogleSignInButton({ className = '', onError }: GoogleSignInButtonProps) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className={`w-full flex items-center justify-center gap-3 py-3.5 px-4 bg-white border border-slate-200 rounded-2xl font-bold text-slate-700 shadow-sm hover:bg-slate-50 hover:border-slate-300 disabled:opacity-60 disabled:cursor-not-allowed transition-all active:scale-[0.98] ${className}`}
    >
